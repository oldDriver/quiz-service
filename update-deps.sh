...
sed -i.bak 's;^# For a PostgreSQL database, use: "postgresql://quiz:quiz@127.0.0.1:5432/quiz?serverVersion=11"$;# For a MySQL database, use: "mysql://db_user:db_password@127.0.0.1:3306/db_name?serverVersion=5.7";' .env
sed -i.bak 's;^DATABASE_URL=mysql://db_user:db_password@127.0.0.1:3306/db_name?serverVersion=5.7$;DATABASE_URL=postgres://quiz:quiz@db/quiz?server_version=12;' .env
